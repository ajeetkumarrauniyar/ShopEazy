...
import { invoiceItemsTable, invoicesTable, syncQueue, type InvoiceWithItems, type NewInvoice, type NewInvoiceItem, type NewSyncQueueItem } from "../db/schema";
...
export async function getInvoiceWithItems(invoiceId: number): Promise<InvoiceWithItems | undefined> {
...
export async function saveInvoice(invoiceData: {
  ...
  try {
    ...
    return await db.transaction(async (tx) => {
      ...
      const [invoice] = await tx.insert(invoicesTable).values(invoiceInsert).returning();
      ...
      const itemInserts: NewInvoiceItem[] = invoiceData.lineItems.map(item => ({
      ...
      console.log(`Invoice ${invoiceData.invoiceNumber} saved successfully with ID: ${invoice.id}`);
...
export async function deleteInvoice(invoiceId: number): Promise<void> {
  try {
    ...
    await addToSyncQueue('invoices', invoiceId, 'DELETE', { invoiceId });
